...
class TaskMode(DomainEnum):
    ...
    INTERVAL = "interval"
    DAILY = "daily"
    #: Раз в неделю в заданный день недели и время. Нужен обслуживающим
    #: задачам, которые привязаны к дню недели, а не к числу дней между
    #: запусками: «каждые 7 дней» после пропуска смещается на другой день.
    WEEKLY = "weekly"
    MANUAL = "manual"
